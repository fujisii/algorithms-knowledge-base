...
最初に迷いやすいのは `left <= right` と `left < right` の違い。`<=` にしないと、配列が1要素のとき（`left == right`）に中央を一度も見ずに終わる。また、中央インデックスを `(left + right) / 2` で計算すると整数オーバーフローが起きるケースがある（Java/C/Go 等の固定幅整数では問題になる）。JavaScript/TypeScript の数値はすべて浮動小数点なので実用上は問題ないが、Go 実装では `left + (right-left)/2` を使ってオーバーフローを回避している。
