...
各アルゴリズムディレクトリ（例: `algorithms/bubble-sort/go/`）は `run.go` に `func main()` を持ち、`go run .` で単体実行できる構成にしている。この都合上、同ディレクトリの全ファイル（実装・テスト）は `package main` を使用する。

ライブラリとして再利用する場合は `package bubblesort` 等の専用パッケージに切り出すことが Go の慣習だが、本リポジトリでは学習用の直接実行を優先してこの構成を採用している。
